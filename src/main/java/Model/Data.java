...
public class Data {
    ...
    public byte[] useSBOX(byte[] bits) {
        ...
        for (int i = 0; i < SBOX.length; i++) {
            ...
            byte firstBit = bits[i * 6];
            byte lastBit = bits[i * 6 + 5];

            byte rowValue = (byte) (((firstBit & 1) << 1) | (lastBit & 1));
            byte colValue = (byte) (
                    ((bits[i * 6 + 1] & 1) << 3) |
                            ((bits[i * 6 + 2] & 1) << 2) |
                            ((bits[i * 6 + 3] & 1) << 1) |
            ...
            int val = SBOX[i][rowValue][colValue];

            for (int j = 0; j < 4; j++) {
                newBits[i * 4 + j] = (byte) ((val >> (3 - j)) & 1);
            }
        }
